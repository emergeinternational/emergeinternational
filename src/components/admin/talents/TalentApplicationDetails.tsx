...
interface TalentApplicationDetailsProps {
  application: TalentApplication;
}

export const TalentApplicationDetails = ({ application }: TalentApplicationDetailsProps) => {
  return (
      ...
      <div className="flex items-center space-x-4">
        <Avatar className="h-20 w-20">
          {application.photo_url ? (
            ...
            <AvatarFallback>
              {application.full_name.substring(0, 2).toUpperCase()}
            </AvatarFallback>
          )}
        </Avatar>
        <div>
            ...
            {application.social_media && (
              <div className="space-y-1">
                ...
                {application.social_media.tiktok && (
                  <p className="text-sm">TikTok: {application.social_media.tiktok}</p>
                )}
                {application.social_media.telegram && (
                  <p className="text-sm">Telegram: {application.social_media.telegram}</p>
                )}
              </div>
            )}
          </div>
